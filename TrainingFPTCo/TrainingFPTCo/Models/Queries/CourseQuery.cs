...
namespace TrainingFPTCo.Models.Queries
{
	public class CourseQuery
	{
		...
		public List<CourseDetail> GetAllDataCourses(string SearchString, string FilterStatus)
		{
			...
			using (SqlConnection connection = Database.GetSqlConnection())
			{
				...
				// Thêm điều kiện tìm kiếm nếu có từ khóa tìm kiếm được cung cấp
				if (!string.IsNullOrEmpty(SearchString))
				{
					sql += " AND ([co].[Name] LIKE @search OR [ca].[Name] LIKE @search)";
				}
				...
				// Thêm tham số cho từ khóa tìm kiếm nếu có
				if (!string.IsNullOrEmpty(SearchString))
				{
					cmd.Parameters.AddWithValue("@search", "%" + SearchString + "%");
				}

				// Thêm tham số cho bộ lọc trạng thái nếu có
				if (!string.IsNullOrEmpty(FilterStatus))
				{
		...
		public bool DeleteItemCourseById(int id = 0)
        {
